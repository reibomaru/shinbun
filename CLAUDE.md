...
- **UIの変更時はスクリーンショットで必ず確認すること。** `screenshot` スキルを利用して、期待している画面と一致しているかを確認する。
- **操作者にも変更内容がわかるように、開発サーバーを立ち上げておくこと。**
- ポート割り当て:
  - mock: `4000`
  - frontend: `3000`
- **ポートが空いていない場合は、ポート番号をずらして起動すること。** 既存のポートを利用しているプロセスを kill してはならない。
