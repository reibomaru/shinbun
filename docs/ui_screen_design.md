...
`is_urgent = true` かつ **未読**（`read_status = null`）の記事が存在する場合、フィルターバーの上に赤いバナーを表示する。バナー全体がクリッカブルで記事詳細へ遷移する。
...
ユーザーがアラートを確認（Acknowledge）すると、以下の2段階で処理される:

1. **楽観的UI更新:** `dismissedIds` state に記事IDを追加し、即座にバナーから非表示にする（ページリロード前の即時フィードバック）
2. **DB永続化:** `markAsRead()` Server Action を呼び出し、`read_status` カラムに既読状態を書き込む。これにより、ページリロード後も確認済みアラートは再表示されない。
