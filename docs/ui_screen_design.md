...
ページ右上に常時表示するボタン:
- **あとで読む**（Clock アイコン）: `saved_item` テーブルに追加（ReadLaterButton コンポーネント）。保存済みの場合は Check アイコン（青）に変化。トースト通知で結果をフィードバック。
- **役立った**（ThumbsUp アイコン）: `feedback` テーブルに `helpful` として記録（FeedbackButtons コンポーネント）。押下後はアイコンが塗りつぶしに変化し、緑色背景でアクティブ状態を表示。トースト通知あり。
- **不要**（ThumbsDown アイコン）: `feedback` テーブルに `not_helpful` として記録。押下後は赤色背景でアクティブ状態を表示。トースト通知あり。

フィードバックボタン（役立った / 不要）は一度押すと再押下不可（disabled）。すべてのボタン操作で sonner によるトースト通知を表示する。
