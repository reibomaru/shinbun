...
- ナビゲーション: ダッシュボード / アーカイブ / 保存済み（現在地をハイライト）
- 全文検索ボックス
- 現在日付（右端）
...
スクロールに追従する（`lg:sticky lg:top-20`）リリース専用カラム。モバイルではメインコンテンツの下に自然に配置される（sticky なし）。各エントリは:
- リリース名（太字）
- ソース名 · 公開日時
- 重要度スコア（★数値）
...
**レスポンシブ対応:**
- アクションボタンはモバイルで `h-11`（44px）のタッチターゲットを確保、デスクトップでは `h-8`（32px）
- ラベル（タグチップ）の `py` を `1` に拡大し、タップしやすさを向上
...
**レイアウト:** 最大幅 `max-w-4xl`（中央寄せ）、3カラムグリッド（`sm:grid-cols-2 lg:grid-cols-3`）
...
- **記事詳細・保存済みページ:** ヘッダー部コントロールが `sm:` 未満で縦積み
...
- 保存済みページのタグチップ: モバイル `min-h-[44px]` / デスクトップ通常サイズ
